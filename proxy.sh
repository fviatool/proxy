...
# Function to check if the entered IPv6 is valid
checkIPv6() {
  local ipv6=$1
  if [[ "$ipv6" =~ ^([a-fA-F0-9]{1,4}::?){1,7}[a-fA-F0-9]{1,4}$ ]]; then
    echo "Valid"
  else
    echo "Invalid"
  fi
...
getIPv6() {
  # Using ifconfig to get IPv6 address of a network interface (e.g., eth0)
  ipv6=$(ifconfig eth0 | grep -oP '(?<=inet6\s)[a-f0-9:]+')
  echo "$ipv6"
}
...
# Set main IP
  ...
  ipv6=$(readWithTrim)
  if [[ "$(checkIPv6 $ipv6)" == "Invalid" ]]; then
    echo "Dia chi IPv6 khong hop le. Vui long nhap lai."
    exit 1
  fi
  if [[ "$ipv6" == *\/* ]]; then
...
if [[ "$OS" = "CentOS Linux" ]]; then
  ...
  if grep -q IPV6ADDR_SECONDARIES "/etc/sysconfig/network-scripts/ifcfg-eth0"; then
    ...
    echo "IPV6ADDR_SECONDARIES=\"${ipv6}/${ipv6mask}\"" >> /etc/sysconfig/network-scripts/ifcfg-eth0
    echo "IPV6_DEFAULTGW=${ipv6gw}" >> /etc/sysconfig/network-scripts/ifcfg-eth0
  fi
fi
systemctl restart network
...
systemctl restart network
  ...
  # Tao IPv6 proxy
    ...
    if [[ "$GWCheck" = "NOGW" ]]; then
      ...
      echo -e ""
      echo -e "Chon loai proxy:"
      echo -e "1. Proxy co mat khau"
      echo -e "2. Proxy khong mat khau"
      read -p "Nhap lua chon cua ban: " proxyType
      if [[ "$proxyType" = "1" ]]; then
        ...
        generateDataNoPass $noProxyIPv6 $ipv4 $prefix $ipv6mask >> $WORKDATA
      fi
    ...
    if [[ -f ${WORKDATA} ]] ; then
      printf "%15s %8s    %15s\n" "IPv4" "Port" "IPv6"
      echo -e "==========================================="
      awk -F "/" 'BEGIN{ORS="";} {printf "%15s %8s >> %15s\n",$3,$4,$5}' ${WORKDATA}
    fi
